...
### 2.2. Resource
- **get_time_range_by_market** : KRX-Stock 서버에서 조회 가능한 정보의 날짜 범위를 제공합니다. 예를 들어, KOSPI의 경우 2010년 1월 4일부터 현재 기준 하루 전날 정보까지 조회할 수 있습니다. *(이 기능은 v0.3.0에서 삭제될 예정입니다.)*


## 3. Enhancements

### 3.1. Date Watcher
**Date Watcher**는 날짜(KST)를 모니터링합니다. 이를 통해 사용자가 질의에 구체적인 날짜를 밝히지 않아도 최신 상태의 정보를 받을 수 있도록 합니다.

### 3.2. Cache
KRX API 요청을 최소화하기 위해 각각의 툴마다 대응하는 **LRU 캐시**를 가집니다. *'(날짜, 시장)'* 을 키로 데이터를 저장하며, 최신 상태의 정보는 항상 저장하고 있습니다. 
...
### 4.2. Health Check
Claud Desktop 에서는 Date Watcher, Cache 등 Krx-Stock 서버의 내부 동작을 파악하기 쉽지 않습니다. 서버의 동작을 확인하기 위해 `health_check.py` 파일을 실행해 서버에 직접 요청을 보낼수 있습니다.
```
# 서버 실행
uv run main.py --transport http-stramable
